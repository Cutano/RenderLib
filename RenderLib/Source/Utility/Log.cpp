...
namespace RL
{
	...
	std::shared_ptr<spdlog::logger> Log::GraphicsEngineLogger()
	{
		...
		std::vector<spdlog::sink_ptr> sinks =
		{
			std::make_shared<spdlog::sinks::basic_file_sink_mt>("logs/GraphicsEngine.log", true),
			std::make_shared<spdlog::sinks::wincolor_stdout_sink_mt>()
		};

		s_Instance = std::make_shared<spdlog::logger>("GraphicsEngine", sinks.begin(), sinks.end());
		return s_Instance;
	}

	std::shared_ptr<spdlog::logger> Log::ScriptCoreLogger()
	{
		static std::shared_ptr<spdlog::logger> s_Instance {nullptr};
		if (s_Instance)
		{
		...
		std::vector<spdlog::sink_ptr> sinks =
		{
			std::make_shared<spdlog::sinks::basic_file_sink_mt>("logs/ScriptCore.log", true),
			std::make_shared<spdlog::sinks::wincolor_stdout_sink_mt>()
		};

		s_Instance = std::make_shared<spdlog::logger>("ScriptCore", sinks.begin(), sinks.end());
		return s_Instance;
	}
